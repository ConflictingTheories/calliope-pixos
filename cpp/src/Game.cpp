...
void Game::update(float deltaTime) {
    ...
    // Handle keyboard for abilities
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) castAbility(0);
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) castAbility(1);
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) castAbility(2);
}
...
void Game::castAbility(int abilityIndex) {
    if (!selectedUnit || abilityIndex >= abilities.size()) return;
    const auto& ability = abilities[abilityIndex];
    if (selectedUnit->mp < ability.mpCost) return;
