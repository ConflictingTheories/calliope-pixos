...
class Authenticator {
  ...
  static isAuthenticated() {
    if (!sessionStorage.user && !localStorage.user) {
      ...
      try {
        var PixosUser = JSON.parse(sessionStorage.user || localStorage.user);
        if (PixosUser && PixosUser.is_authorized) return true;
        else return false;
      } catch (e) {
        Authenticator.clearAuthData();
        return false;
  ...
  static isAdmin() {
    if (!sessionStorage.user && !localStorage.user) {
      ...
      try {
        var PixosUser = JSON.parse(sessionStorage.user || localStorage.user);
        if (PixosUser && PixosUser.is_admin) return true;
        else return false;
      } catch (e) {
        Authenticator.clearAuthData();
        return false;
