...
export async function check() {
  return ApiHelper.get(`/auth`).then((user: any) => {
...
export async function login(username: string, password: string) {
  ...
  return ApiHelper.post(`/auth/login`, reqBody).then((user: any) => {
...
export async function logout() {
  return ApiHelper.post(`/auth/logout`).then(() => {
