 ...
 */
function MapEditor({ content, onSave, tileset, geometry, tiles, textureAtlas, zip, cells: initialCells, heights: initialHeights }) {
  console.log('[MapEditor3D] mount props:', {
    hasContent: !!content,
    tilesetName: tileset?.name || null,
    tilesCount: tiles ? Object.keys(tiles).length : 0,
    geometryCount: geometry ? Object.keys(geometry).length : 0,
    hasTextureAtlas: !!textureAtlas,
    initialCellsLen: initialCells ? initialCells.length : 0,
    initialHeightsLen: initialHeights ? initialHeights.length : 0,
  });
  // Map state
  ...
  // History helper: push a snapshot (stable via useCallback)
  const pushHistory = useCallback((newCells, newHeights) => {
    try {
      const snapshot = {
      ...
      console.error('[MapEditor3D] pushHistory failed:', e);
    }
  ...
  useEffect(() => {
    ...
    try {
      let parsedMap = content;
      let parsedCells = initialCells || [];
      let parsedHeights = initialHeights || [];

      setMap(parsedMap);
      setCells(parsedCells);
      setHeights(parsedHeights);
      console.log('[MapEditor3D] parsed map content:', {
        cellsRows: parsedCells ? parsedCells.length : 0,
        heightsRows: parsedHeights ? parsedHeights.length : 0,
        hasSprites: parsedMap?.sprites ? true : false,
        hasObjects: parsedMap?.objects ? true : false,
      });
      ...
      if (parsedCells) {
        if (typeof pushHistory === 'function') {
          ...
          console.warn('[MapEditor3D] pushHistory not available at init');
        }
      }
      setError(null);
    } catch (err) {
  ...
  }, [content, initialCells, initialHeights]);

  // Load texture atlas
  useEffect(() => {
    console.log('[MapEditor3D] textureAtlas prop changed:', textureAtlas ? (typeof textureAtlas === 'string' ? `${textureAtlas.slice(0, 64)}...` : textureAtlas) : null);
    if (!textureAtlas || !textureAtlas.startsWith('data:')) {
      return;
    }
    ...
    img.onload = () => {
      if (glRef.current) {
        ...
        console.log('[MapEditor3D] Texture atlas applied to GL texture');
  ...
  // Load individual texture images referenced by the tileset if they refer to actual image files
  useEffect(() => {
    if (!tileset || !zip || !glRef.current) return;
    const textureDefs = tileset.textures || {};

    const isImagePath = (val) => typeof val === 'string' && /\.(png|jpg|jpeg|gif|bmp)$/i.test(val);

    const tryLoadImage = async (name, path) => {
      try {
        let entry = null;
        if (zip.files && zip.files[path]) entry = zip.files[path];
        else if (zip.root) {
          // Traverse root to find entry with matching full path
          const findEntry = (node, currentPath = '') => {
            if (!node || !node.children) return null;
            for (const child of node.children) {
              const fp = currentPath ? `${currentPath}/${child.name}` : child.name;
              if (!child.directory && fp === path) return child;
              const found = findEntry(child, fp);
              if (found) return found;
            }
            return null;
          };
          entry = findEntry(zip.root, '');
        }
        if (!entry) return;
        const imgBytes = await (entry.async ? entry.async('blob') : getData(entry, false));
        // Create data URI
        let dataUri = null;
        if (typeof imgBytes === 'string') dataUri = imgBytes;
        else {
          const ext = (path.split('.').pop() || 'png').toLowerCase();
          const mime = `image/${ext === 'jpg' ? 'jpeg' : ext}`;
          const reader = new FileReader();
          const blob = imgBytes instanceof Blob ? imgBytes : new Blob([imgBytes], { type: mime });
          await new Promise((res) => { reader.onload = () => res(); reader.readAsDataURL(blob); });
          dataUri = reader.result;
        }
        // Load into image and create GL texture
        const img = new Image();
        img.onload = () => {
          const tex = createTextureFromImage(glRef.current, img);
          texturesCacheRef.current[name] = tex;
        };
        img.onerror = (e) => {
          console.warn('[MapEditor3D] Failed to load individual texture:', path, e);
        };
        img.src = dataUri;
      } catch (e) {
        console.warn('[MapEditor3D] Error loading individual tile texture:', name, path, e);
      }
    };

    // For each texture def that is a path, try to load it
    for (const [tName, def] of Object.entries(textureDefs)) {
      if (isImagePath(def)) {
        tryLoadImage(tName, def);
      }
    }
  }, [tileset, zip]);

  // Show error state if tileset data is missing for the loaded map
  useEffect(() => {
    if (!map) return;
    if (!tiles || Object.keys(tiles).length === 0) {
      const msg = 'Map loaded but tileset data is missing. 3D view will be empty. Expected tileset: ' + (map?.tileset || 'unknown');
      setError(prev => (prev === msg ? prev : msg));
    } else {
      // Clear the specific tileset error if tiles are present
      setError(null);
    }
  }, [map, tiles]);
    ...
    (gl, projectionMatrix, viewMatrix, camera, showGridFlag) => {
      if (!shaderProgramRef.current || !cells.length) {
        console.log('[MapEditor3D] Early return - no shader program or no cells', {
          programPresent: !!shaderProgramRef.current,
          cellsLength: cells ? cells.length : 0,
          tilesCount: tiles ? Object.keys(tiles).length : 0,
          textureAtlasPresent: !!textureRef.current,
      ...
      for (let y = 0; y < cells.length; y++) {
        for (let x = 0; x < cells[y].length; x++) {
          const rawTile = cells[y][x];
          let tileType = rawTile;
          if (typeof rawTile === 'number' && tilesByIdRef.current[rawTile]) {
            tileType = tilesByIdRef.current[rawTile];
          } else if (typeof rawTile === 'string' && tiles && !tiles[rawTile]) {
            const maybe = Number(rawTile);
            if (!Number.isNaN(maybe) && tilesByIdRef.current[maybe]) {
  ...
  function renderTile(
    ...
    for (let i = 0; i < tileData.length; i += 3) {
      ...
      if (!geometrySafe || !geometrySafe[geometryName]) continue;

      const geom = geometrySafe[geometryName];
      renderGeometry(
        gl,
        program,
        geom,
  ...
  function renderGeometry(
    ...
    if (textureName && textureRef.current && tileset?.textures?.[textureName]) {
      ...
      // If there is an individual texture loaded for this textureName, bind it instead
      const singleTex = texturesCacheRef.current[textureName];
      if (textureName && singleTex) {
        gl.uniform1i(uUseTexture, 1);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, singleTex);
        gl.uniform1i(uTexture, 0);
      } else {
        if (textureName && !singleTex) {
          // Warn only once per missing texture, include extra diagnostic info
          if (!missingTextureWarnedRef.current.has(textureName)) {
            missingTextureWarnedRef.current.add(textureName);
            console.warn('[MapEditor3D] Texture requested but not loaded:', textureName, {
              textureRefLoaded: !!textureRef.current,
              tilesetName: tileset?.name,
              tilesetHasTextures: !!(tileset?.textures && Object.keys(tileset.textures || {}).length),
              hasTilesetTextureKey: !!(tileset?.textures && tileset.textures[textureName] !== undefined),
            });
          }
        }
    ...
    const handleCellClick = useCallback(
      (screenX, screenY, camera, event) => {
        ...
        const cellCoords = screenToCell(screenX, screenY, camera, canvas);
        if (!cellCoords) {
          return;
        }
    ...
    function screenToCell(screenX, screenY, camera, canvas) {
      ...
      if (!canvas) return null; // defensive: canvas may not be available until GL init
    ...
    // History management: using pushHistory defined above via useCallback
    ...
    // Build a map of tile id -> tile name if tiles use numeric ids
    const tilesByIdRef = useRef({});
    useEffect(() => {
      const map = {};
      ...
      tilesByIdRef.current = map;
    }, [tiles]);

    // Show loading/error states
    if (!content) {
      // No content selected — show the 'no map' message
      return (
    ...
    // If a map has been selected but cells haven't been loaded yet, show a loading indicator instead
      ...
      return (
        <div style={{ padding: '1rem', background: '#1e1e1e', color: '#d4d4d4', minHeight: '100vh' }}>
          <div style={{
            ...
            ⌛ Loading map assets... Please wait.
          </div>
        </div>
      );
    }
    // Avoid setting state during render - move tileset-missing error into an effect below

    // Ensure geometry is at least an object so downstream code doesn't crash
    const geometrySafe = geometry || {};

    // use geometrySafe where needed to avoid mutating props
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span>📐 Geometry ({Object.keys(geometrySafe).length})</span>
            </div>
            <div style={{ padding: '10px' }}>
              {/* Inline Geometry Inspector */}
              {editingGeometryName && geometrySafe[editingGeometryName] && (
                <div style={{
                  background: '#1e1e2e',
                  ...
                  <div style={{ fontSize: '11px', color: '#aaa' }}>
                    Vertices: {geometrySafe[editingGeometryName]?.vertices?.length || 0} |
                    Surfaces: {geometrySafe[editingGeometryName]?.surfaces?.length || 0} |
                    Type: {geometrySafe[editingGeometryName]?.type_bitmask || 0}
                  </div>
                  {geometrySafe[editingGeometryName]?.vertices && (
                    <div style={{ marginTop: '8px', fontSize: '10px', color: '#888' }}>
                      {geometrySafe[editingGeometryName].vertices.slice(0, 6).map((v, i) => (
                        <div key={i} style={{ fontFamily: 'monospace' }}>
                          v{i}: [{v.map(n => n.toFixed(2)).join(', ')}]
                        </div>
                      ))}
                      {geometrySafe[editingGeometryName].vertices.length > 6 && (
                        <div style={{ fontStyle: 'italic' }}>... {geometrySafe[editingGeometryName].vertices.length - 6} more</div>
                      )}
                    </div>
                  )}
                </div>
              ...
              <div style={{ maxHeight: '150px', overflowY: 'auto', marginBottom: '10px' }}>
                {Object.keys(geometrySafe).sort().map(geomName => (
                  <div
                    key={geomName}
                  ...
                  >
                    ...
                    <span style={{ color: '#888', fontSize: '10px' }}>
                      {geometrySafe[geomName]?.vertices?.length || 0} △
                    </span>
                  </div>
                ))}
              </div>
  ...
  // If a map content is present but there are no cells, show a helpful message
  if (content && (!cells || cells.length === 0)) {
    return (
      <div style={{ padding: '1rem', background: '#1e1e1e', color: '#d4d4d4', minHeight: '100vh' }}>
        <div style={{
          background: '#7a1a1a',
          ...
          ⚠️ Map loaded but contains no cell data. The map either relies on a separate <code>cells.json</code> file that wasn't found or the map JSON doesn't include inline cells.
          <div style={{ marginTop: '8px' }}>Expected tileset: <strong>{content?.tileset}</strong></div>
          <div style={{ marginTop: '8px' }}>Try adding a <code>cells.json</code> beside the <code>map.json</code> or include <code>cells</code> inline in the map.</div>
        </div>
      </div>
    );
  }
