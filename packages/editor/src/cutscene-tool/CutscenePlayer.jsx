...
function parseScript(text) {
  ...
  while (i < lines.length) {
    ...
    if (raw.startsWith('wait ')) {
      ...
      if (mm)
        scene.events.push({ type: 'wait', payload: { ms: parseInt(mm[1], 10) }, blocking: true });
      i++;
      continue;
    }
...
const CutscenePlayer = forwardRef(
  (
    {
      scriptText,
      speed = 60,
    ...
    async function internalExport() {
      ...
      if (!html2canvas) {
        alert(
          'Video export requires html2canvas library. Please install it with: npm install html2canvas'
        );
        return null;
      }
      ...
      try {
        ...
        if (onExportProgress)
          onExportProgress({ status: 'recording', progress: 5, message: 'Starting playback...' });
        ...
        debug(
          'CutscenePlayer',
          '[Export] Playback complete, captured',
          frameCountRef.value,
          'frames'
        );
        ...
        if (onExportProgress)
          onExportProgress({ status: 'processing', progress: 92, message: 'Processing video...' });

        debug(
          'CutscenePlayer',
          '[Export] Stopping recorder, state:',
          mediaRecorder.state,
          'chunks so far:',
          recordedChunks.length
        );
        ...
        return new Promise(resolve => {
          mediaRecorder.onstop = async () => {
            debug(
              'CutscenePlayer',
              '[Export] MediaRecorder stopped, total chunks:',
              recordedChunks.length
            );
            ...
            if (blob.size === 0) {
              ...
              if (onExportProgress)
                onExportProgress({
                  status: 'error',
                  progress: 0,
                  message: 'Export failed: No video data captured',
                });
        ...
        if (onExportProgress)
          onExportProgress({ status: 'error', progress: 0, message: error.message });
    ...
    async function showDialogue(ev) {
          ...
          try {
            ...
            return new Promise(resolve => {
              ...
              audio
                ...
                .catch(err => {
                  console.warn(
                    '[CutscenePlayer] Dialogue voice-over playback prevented:',
                    voiceFile,
                    err.message
                  );
                  resolve();
      ...
      if (sprite) {
        ...
        if (sprite.endsWith('.json')) {
          ...
          const isSpriteRef =
            sprite.includes('characters/') ||
            sprite.includes('npc/') ||
            sprite.includes('monsters/');
        ...
        return new Promise(async resolve => {
          try {
            ...
            audio
              ...
              .catch(err => {
                console.warn(
                  '[CutscenePlayer] Voice-over playback prevented:',
                  soundName,
                  err.message
                );
                resolve();
