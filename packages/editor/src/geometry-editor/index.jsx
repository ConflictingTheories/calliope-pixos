...
import { InputNumber, Button, Message, Input } from '../ui';
...
function GeometryPreview({ geometry, size = 280 }) {
  ...
  useEffect(() => {
    ...
    if (geometry?.vertices?.length > 0) {
      ...
      const trianglesWithDepth = geometry.vertices
        .map((tri, idx) => {
...
function GeometryEditor({ content, onSave }) {
  ...
  const addGeometry = useCallback(() => {
    const name =
      newKeyName.trim().toUpperCase().replace(/\s+/g, '_') ||
      `GEOM_${Object.keys(geometryMap).length}`;
    if (geometryMap[name]) {
  ...
  const renameGeometry = useCallback(
    oldKey => {
  ...
  const duplicateGeometry = useCallback(
    key => {
  ...
  const geometryKeys = Object.keys(geometryMap).filter(
    k => !searchFilter || k.toLowerCase().includes(searchFilter.toLowerCase())
  );
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <Input
              ...
              style={{ flex: 1 }}
            />
            <Button appearance="primary" size="xs" onClick={addGeometry}>
              +
            </Button>
          </div>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  marginBottom: '4px',
                  background:
                    selectedKey === key ? 'rgba(125,211,252,0.2)' : 'rgba(255,255,255,0.03)',
          ...
          <>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div
                  style={{ color: 'rgba(255,255,255,0.5)', fontSize: '11px', marginBottom: '12px' }}
                >
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            ...
                            const newSurfs = (selectedGeom.surfaces || []).filter(
                              (_, i) => i !== triIdx
                            );
                            updateGeometry(selectedKey, 'vertices', newVerts);
                            updateGeometry(selectedKey, 'surfaces', newSurfs);
                          ...
                          {triangle.map((vertex, vIdx) => (
                            ...
                            >
                              <span
                                style={{
                                  color:
                                    vIdx === 0 ? '#ef4444' : vIdx === 1 ? '#22c55e' : '#3b82f6',
                                  fontSize: '10px',
                                  width: '16px',
                                  fontWeight: 600,
                                }}
                              ...
                              {vertex.map((coord, cIdx) => (
                                <InputNumber
                                  ...
                                  step={0.1}
                                  onChange={val => {
                                    const newVerts = JSON.parse(
                                      JSON.stringify(selectedGeom.vertices)
                                    );
                            ...
                            >
                              ...
                              {uv.map((coord, cIdx) => (
                                <InputNumber
                                  ...
                                  step={0.1}
                                  onChange={val => {
                                    const newSurfs = JSON.parse(
                                      JSON.stringify(selectedGeom.surfaces || [])
                                    );
                                    if (!newSurfs[triIdx])
                                      newSurfs[triIdx] = [
                                        [0, 0],
