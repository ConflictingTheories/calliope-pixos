...
import { LuaType, Config, coerceArgToString } from '../utils.js';
...
const getLibPackage = (
  ...
  const searchers = new Table([
    ...
    (
      moduleName: string
    ): [undefined] | [string] | [(modname: string, path: string) => LuaType, string] => {
      ...
      return [
        (moduleName: string, path: string) => execModule(cfg.loadFile(path), moduleName),
        res as string,
      ];
    },
  ]);
