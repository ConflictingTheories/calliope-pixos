...
function addPhysicsBody(entity: any, width: LuaType, height: LuaType, opts: any = {}) {
  ...
  if (opts && typeof opts === 'object') {
    if ((opts as any).mass !== undefined)
      options.mass = coerceArgToNumber((opts as any).mass, 'addPhysicsBody', 4);
    if ((opts as any).friction !== undefined)
      options.friction = coerceArgToNumber((opts as any).friction, 'addPhysicsBody', 4);
    if ((opts as any).restitution !== undefined)
      options.restitution = coerceArgToNumber((opts as any).restitution, 'addPhysicsBody', 4);
    if ((opts as any).useGravity !== undefined)
      options.useGravity = Boolean((opts as any).useGravity);
    ...
    if ((opts as any).layer !== undefined)
      options.layer = coerceArgToNumber((opts as any).layer, 'addPhysicsBody', 4);
    if ((opts as any).mask !== undefined)
      options.mask = coerceArgToNumber((opts as any).mask, 'addPhysicsBody', 4);
  }
...
function queryCollisions(x: LuaType, y: LuaType, width: LuaType, height: LuaType) {
  ...
  const aabb = new AABB(new Vector(x, y, 0), new Vector(x + width, y + height, 1));

  const candidates = (global as any).engine.physics.spatialHash.getPotentialCollisions({
    getAABB: () => aabb,
  });
