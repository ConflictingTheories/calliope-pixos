...
const generate = (node: luaparse.Node): string | MemExpr => {
  switch (node.type) {
    ...
    case 'StringCallExpression': {
      ...
      if (
        functionName instanceof MemExpr &&
...
const parseBody = (node: Block, header = ''): string => {
  ...
  const scopeDef =
    scope === undefined ? '' : `const $${scope} = $${scopeToParentScope.get(scope)}.extend();`;
...
const parseAssignments = (node: luaparse.LocalStatement | luaparse.AssignmentStatement): string => {
  ...
  const useTempVar =
    node.variables.length > 1 && node.init.length > 0 && !node.init.every(isLiteral);
        ...
        switch (n.type) {
          ...
          case 'LabelStatement': {
            if (
              gotoInfo.find(
                node =>
                  node.type === 'label' && node.name === n.label.name && node.scope === gotoScope
              )
            ) {
  ...
  for (let i = 0; i < gotoInfo.length; i++) {
    ...
    if (goto.type === 'goto') {
      const label = gotoInfo
        .filter(
          node => node.type === 'label' && node.name === goto.name && node.scope <= goto.scope
        )
...
const setExtraInfo = (ast: luaparse.Chunk): void => {
  ...
  const visitProp = (node: luaparse.Node, prevScope: number, prevGoto: number): void => {
    ...
    if (isBlock(node)) {
      ...
      // set goto info
      if (
        node.body.findIndex(s => s.type === 'LabelStatement' || s.type === 'GotoStatement') !== -1
      ) {
        nextGoto = gotoID;
