...
export const updateWebglDebugInformation = self => {
  if (self.showWebglDebug && self.webglDebugDiv) {
    ...
    self.webglDebugDiv.innerHTML =
      ...
      '<br>' +
      'Sprites Drawn: ' +
      (debug.spritesDrawn || 0) +
      '<br>' +
      'Objects Drawn: ' +
      (debug.objectsDrawn || 0) +
      '<br>' +
      'Renderer: ' +
      renderer +
      '<br>' +
  ...
  window.addEventListener('pixos:freecam:toggle', () => {
    ...
    const onPointerMove = me => {
      ...
      try {
        ...
        if (rm.camera) {
          ...
          rm.camera.pitch = Math.max(
            -Math.PI / 2 + 0.01,
            Math.min(Math.PI / 2 - 0.01, rm.camera.pitch + pitchDelta)
          );
    ...
    const tick = () => {
      ...
      try {
        ...
        const vm =
          rm.camera && rm.camera.uViewMat
            ? Array.from(rm.camera.uViewMat)
                .slice(0, 8)
                .map(n => n.toFixed(3))
            : [];
        statusEl.innerText =
          ...
          '\n' +
          'pointerLock: ' +
          (document.pointerLockElement === canvas ? 'yes' : 'no') +
          '\n' +
          'pos: ' +
          JSON.stringify(pos) +
          ' yaw:' +
    ...
    if (self.showFreeCam) {
      ...
      try {
        self._freeCamSavedState = {
          position: new Vector(
            rm.camera.cameraPosition.x,
            rm.camera.cameraPosition.y,
            rm.camera.cameraPosition.z
          ),
          yaw: rm.camera.yaw,
          pitch: rm.camera.pitch,
          distance: rm.camera.cameraDistance,
          target: rm.camera.cameraTarget
            ? new Vector(
                rm.camera.cameraTarget.x,
                rm.camera.cameraTarget.y,
                rm.camera.cameraTarget.z
              )
            : null,
          viewMat: create(),
        };
        ...
        try {
          console.log(
            '[FreeCam] ENTER - savedState.viewMat:',
            Array.from(self._freeCamSavedState.viewMat)
          );
        } catch (err) {}
      } catch (err) {}
      ...
      info.innerHTML =
        'FREE CAM (F5 to exit) &nbsp; WASD/Arrows: move & strafe &nbsp; Q/E: yaw &nbsp; R/F: pitch &nbsp; Wheel: zoom';
      ...
      const onPointerLockChange = () => {
        ...
        try {
          ...
          if (s)
            s.innerText =
              'Keys: ' +
              JSON.stringify((self.keyboard && self.keyboard.activeCodes) || []) +
              ' | pointerLock: ' +
              (locked ? 'yes' : 'no');
          try {
            console.log(
              '[FreeCam] pointerLock change - locked:',
              locked,
              'viewMat:',
              Array.from(rm.camera.uViewMat)
            );
      ...
      if (self._freeCamSavedState) {
        try {
          try {
            console.log(
              '[FreeCam] EXIT - restoring savedState.viewMat:',
              Array.from(self._freeCamSavedState.viewMat)
            );
          } catch (err) {}
          ...
          try {
            if (rm.camera) {
              if (typeof self._freeCamSavedState.yaw !== 'undefined')
                rm.camera.yaw = self._freeCamSavedState.yaw;
              if (typeof self._freeCamSavedState.pitch !== 'undefined')
                rm.camera.pitch = self._freeCamSavedState.pitch;
              if (typeof self._freeCamSavedState.distance !== 'undefined')
                rm.camera.cameraDistance = self._freeCamSavedState.distance;
              if (self._freeCamSavedState.target)
                rm.camera.cameraTarget = self._freeCamSavedState.target;
              rm.camera.updateViewFromAngles && rm.camera.updateViewFromAngles();
            }
          } catch (err) {}
        } catch (err) {}
        ...
        try {
          try {
            console.log(
              '[FreeCam] EXIT - restoring _freeCamSaved:',
              Array.from(self._freeCamSaved)
            );
          } catch (err) {}
      ...
      const info = document.getElementById('pixos-freecam-info');
        ...
        try {
          if (self._freecamHandlers.onPointerLockChange) {
            document.removeEventListener(
              'pointerlockchange',
              self._freecamHandlers.onPointerLockChange
            );
            document.removeEventListener(
      ...
      try {
        document.body.style.overflow = self._bodyOverflowSaved ?? '';
      } catch (err) {}
      try {
        const s = document.getElementById('pixos-freecam-status');
