...
export default class EffectManager {
  ...
  beginScene() {
    console.log(
      `EffectManager.beginScene: activeEffects=${this.activeEffects.length}, effects: ${this.activeEffects.join(', ')}`
    );
  ...
  endScene(timestamp) {
    ...
    for (let i = 0; i < this.activeEffects.length; i++) {
      ...
      if (!isLast) {
        currentSourceTex =
          currentDest === this.pingPongFBO ? this.pingPongTexture : this.sceneTexture;
