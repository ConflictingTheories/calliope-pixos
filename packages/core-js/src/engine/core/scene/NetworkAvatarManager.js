  ...
  addRemoteAvatar(clientId, avatarData) {
    ...
    try {
      ...
      if (this.remoteAvatars.has(clientId)) {
        ...
        try {
          debug(
            'NetworkAvatarManager',
            `Remote avatar for ${clientId} already exists, updating instead`
          );
        } catch (e) {}
      ...
      if (localTemplate) {
        ...
        if (localTemplate.speech && localTemplate.speechTexBuf)
          ((avatar.speech = localTemplate.speech),
      ...
      const zone =
        this.world.getZoneById(avatarData.zone || avatarData.zoneId) ||
        this.world.zoneContaining(avatarData.x || 0, avatarData.y || 0);
      avatar.zone = zone;
      avatar.id = spriteId;
      ...
      const zVal =
        typeof avatarData.z === 'number'
          ? avatarData.z
          : avatarData.pos && typeof avatarData.pos.z === 'number'
            ? avatarData.pos.z
            : zone
              ? zone.getHeight(hx, hy)
              : 0;
      ...
      avatar.vertexPosBuf = this.engine.renderManager.createBuffer(
        poly,
        this.engine.gl.STATIC_DRAW,
        3
      );
      let texCoords = avatar.getTexCoords();
      avatar.vertexTexBuf = this.engine.renderManager.createBuffer(
        texCoords,
        this.engine.gl.DYNAMIC_DRAW,
        2
      );
      if (avatar.enableSpeech) {
        avatar.speechVerBuf = this.engine.renderManager.createBuffer(
          avatar.getSpeechBubbleVertices(),
          this.engine.gl.STATIC_DRAW,
          3
        );
        avatar.speechTexBuf = this.engine.renderManager.createBuffer(
          avatar.getSpeechBubbleTexture(),
          this.engine.gl.DYNAMIC_DRAW,
          2
        );
      }
      ...
      if (zone) {
        ...
        debug(
          'NetworkAvatarManager',
          `Added remote avatar for client ${clientId} as sprite '${avatar.id}' to zone ${zone.id} at (${avatar.pos.x},${avatar.pos.y},${avatar.pos.z})`
        );
      }
      ...
      try {
        debug(
          'NetworkAvatarManager',
          `Remote avatar map now has ${this.remoteAvatars.size} entries`
        );
      } catch (e) {}
      return avatar;
    } catch (e) {
  ...
  updateRemoteAvatar(clientId, avatarData) {
    ...
    if (avatar) {
      try {
        debug(
          'NetworkAvatarManager',
          `updateRemoteAvatar: client=${clientId} pre pos=${avatar.pos?.x},${avatar.pos?.y},${avatar.pos?.z} loaded=${avatar.loaded} id=${avatar.id} zone=${avatar.zone?.id}`
        );
      } catch (e) {}
      ...
      if (!avatar.loaded) {
        console.warn(
          `Remote avatar ${clientId} was not loaded; forcing loaded=true so renderer will attempt to draw.`
        );
        avatar.loaded = true;
        avatar.templateLoaded = true;
        if (!avatar.texture || typeof avatar.texture.attach !== 'function')
          avatar.texture = { loaded: true, attach: () => {} };
      }
      try {
        debug(
          'NetworkAvatarManager',
          `updateRemoteAvatar: client=${clientId} post pos=${avatar.pos?.x},${avatar.pos?.y},${avatar.pos?.z} loaded=${avatar.loaded} id=${avatar.id} zone=${avatar.zone?.id}`
        );
      } catch (e) {}
      return avatar;
    }
