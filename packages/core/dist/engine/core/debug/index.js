...
exports.updateWebglDebugInformation = exports.updateFlagDebugInformation = exports.updateDebugInformation = exports.attachWebglDebugInfo = exports.attachFlagDebugInfo = void 0;
...
var updateDebugInformation = exports.updateDebugInformation = function updateDebugInformation(self) {
 ...
 * @param {import('../index.js').default} self - The engine core instance.
 */
var updateWebglDebugInformation = exports.updateWebglDebugInformation = function updateWebglDebugInformation(self) {
  if (self.showWebglDebug && self.webglDebugDiv) {
    var now = typeof performance !== 'undefined' ? performance.now() : Date.now();
    var delta = now - self.lastDebugTime;
    var fps = delta > 0 ? (1000.0 / delta).toFixed(1) : '0';
    ...
    var debug = self.renderManager.debug || {};
    self.webglDebugDiv.innerHTML = 'FPS: ' + fps + '<br>' + 'Tiles Drawn: ' + (debug.tilesDrawn || 0) + '<br>' + 'Sprites Drawn: ' + (debug.spritesDrawn || 0) + '<br>' + 'Objects Drawn: ' + (debug.objectsDrawn || 0) + '<br>' + 'Renderer: ' + renderer + '<br>' + 'Vendor: ' + vendor + '<br>' + 'GL Version: ' + version;
  }
};
 ...
 * @param {import('../index.js').default} self - The engine core instance.
 */
var updateFlagDebugInformation = exports.updateFlagDebugInformation = function updateFlagDebugInformation(self) {
  if (self.showFlagDebug && self.flagDebugDiv) {
    self.store.set('Debug::Flag::UpdateTime', Date.now());
    var flags = self.store.all();
    console.log({
      self: self,
      keys: JSON.stringify(flags),
      store: self.store.keys()
    });
    var data = Object.keys(flags).map(function (key) {
 ...
 * @param {import('../index.js').default} self - The engine core instance.
 */
var attachFlagDebugInfo = exports.attachFlagDebugInfo = function attachFlagDebugInfo(self) {
 ...
 * @param {import('../index.js').default} self - The engine core instance.
 */
var attachWebglDebugInfo = exports.attachWebglDebugInfo = function attachWebglDebugInfo(self) {
  var div = document.createElement('div');
  div.style.position = 'absolute';
  div.style.top = '0';
  div.style.left = '0';
  ...
  try {
    var kb = self.keyboard;
    var kbHook = function kbHook(ev, type) {
      ...
      var e = new CustomEvent('pixos:freecam:toggle');
  ...
  window.addEventListener('pixos:freecam:toggle', function () {
    var rm = self.renderManager;
    if (!rm || !rm.camera) return;
    var canvas = self.canvas || document.body;
    var panSpeed = 0.2;
    var rotSpeed = 0.002;

    // wheel handler -> zoom in/out using camera.zoom
    var onWheel = function onWheel(we) {
      try {
        we.preventDefault();
        we.stopPropagation();
      } catch (err) {}
      ;
      var dz = (we.deltaY > 0 ? 1 : -1) * 0.5;
      try {
        rm.camera.zoom && rm.camera.zoom(dz);
        moveCounter++;
      } catch (err) {}
    };

    // pointer move handler (pointer lock) -> rotate view matrix directly
    var onPointerMove = function onPointerMove(me) {
      var mx = (me.movementX || 0) * 0.002;
      var my = (me.movementY || 0) * 0.002;
      try {
        // convert to angular delta and apply via rotateCam
        var yawDelta = -mx * 1.5; // scale for responsiveness
        var pitchDelta = -my * 1.5;
        if (rm.camera) {
    ...
    var _tick = function tick() {
      ...
      var codes = self.keyboard && self.keyboard.activeCodes || [];
      var lower = codes.map(function (c) {
      ...
      try {
        var step = 0.5;
        if (rm.camera) {
          if (lower.indexOf('w') >= 0 || lower.indexOf('arrowup') >= 0) {
            rm.camera.translateCam('UP');
            moveCounter++;
          }
          if (lower.indexOf('s') >= 0 || lower.indexOf('arrowdown') >= 0) {
            rm.camera.translateCam('DOWN');
            moveCounter++;
          }
          if (lower.indexOf('a') >= 0 || lower.indexOf('arrowleft') >= 0) {
            rm.camera.translateCam('LEFT');
            moveCounter++;
          }
          if (lower.indexOf('d') >= 0 || lower.indexOf('arrowright') >= 0) {
            rm.camera.translateCam('RIGHT');
            moveCounter++;
          }
          // Q/E yaw
          if (lower.indexOf('q') >= 0) {
            rm.camera.yaw -= 0.03;
            rm.camera.updateViewFromAngles();
            moveCounter++;
          }
          if (lower.indexOf('e') >= 0) {
            rm.camera.yaw += 0.03;
            rm.camera.updateViewFromAngles();
            moveCounter++;
          }
          ...
          if (lower.indexOf('r') >= 0) {
            rm.camera.pitch = Math.max(-Math.PI / 2 + 0.01, rm.camera.pitch - 0.03);
            rm.camera.updateViewFromAngles();
            moveCounter++;
          }
          if (lower.indexOf('f') >= 0) {
            rm.camera.pitch = Math.min(Math.PI / 2 - 0.01, rm.camera.pitch + 0.03);
            rm.camera.updateViewFromAngles();
            moveCounter++;
          }
        }
      ...
      try {
        ...
        if (firstTick) {
          try {
            console.log('[FreeCam] FIRST TICK viewMat:', Array.from(rm.camera.uViewMat));
          } catch (err) {}
          firstTick = false;
        }
        var pos = cam.cameraPosition ? [cam.cameraPosition.x, cam.cameraPosition.y, cam.cameraPosition.z] : ['n/a'];
        var yaw = typeof cam.yaw !== 'undefined' ? cam.yaw.toFixed(3) : 'n/a';
        var pitch = typeof cam.pitch !== 'undefined' ? cam.pitch.toFixed(3) : 'n/a';
        var vm = rm.camera && rm.camera.uViewMat ? Array.from(rm.camera.uViewMat).slice(0, 8).map(function (n) {
          return n.toFixed(3);
        }) : [];
    ...
    if (self.showFreeCam) {
      ...
      self._freeCamSaved = (0, _matrix.create)();
      // log the incoming view matrix for diagnosis
      try {
        console.log('[FreeCam] ENTER - current viewMat:', Array.from(rm.camera.uViewMat));
      } catch (err) {}
      (0, _matrix.set)(rm.camera.uViewMat, self._freeCamSaved);
      try {
        self._freeCamSavedState = {
          position: new _vector.Vector(rm.camera.cameraPosition.x, rm.camera.cameraPosition.y, rm.camera.cameraPosition.z),
          yaw: rm.camera.yaw,
          pitch: rm.camera.pitch,
          distance: rm.camera.cameraDistance,
          target: rm.camera.cameraTarget ? new _vector.Vector(rm.camera.cameraTarget.x, rm.camera.cameraTarget.y, rm.camera.cameraTarget.z) : null,
          viewMat: (0, _matrix.create)()
        };
        (0, _matrix.set)(rm.camera.uViewMat, self._freeCamSavedState.viewMat);
        try {
          console.log('[FreeCam] ENTER - savedState.viewMat:', Array.from(self._freeCamSavedState.viewMat));
        } catch (err) {}
      } catch (err) {}
      if ((_self$spritz = self.spritz) !== null && _self$spritz !== void 0 && _self$spritz.world) self.spritz.world.isPaused = true;
      ...
      var onPointerLockChange = function onPointerLockChange() {
        ...
        try {
          var s = document.getElementById('pixos-freecam-status');
          if (s) s.innerText = 'Keys: ' + JSON.stringify(self.keyboard && self.keyboard.activeCodes || []) + ' | pointerLock: ' + (locked ? 'yes' : 'no');
          try {
            console.log('[FreeCam] pointerLock change - locked:', locked, 'viewMat:', Array.from(rm.camera.uViewMat));
          } catch (err) {}
        } catch (err) {}
      };
      var captureClick = function captureClick(ev) {
      ...
      try {
        self._bodyOverflowSaved = document.body.style.overflow;
      ...
      if (self._freeCamSavedState) {
        try {
          try {
            console.log('[FreeCam] EXIT - restoring savedState.viewMat:', Array.from(self._freeCamSavedState.viewMat));
          } catch (err) {}
          // restore raw view matrix snapshot
          (0, _matrix.set)(self._freeCamSavedState.viewMat, rm.camera.uViewMat);
        ...
        try {
          try {
            console.log('[FreeCam] EXIT - restoring _freeCamSaved:', Array.from(self._freeCamSaved));
          } catch (err) {}
          (0, _matrix.set)(self._freeCamSaved, rm.camera.uViewMat);
          try {
            rm.camera.setFromViewMatrix(rm.camera.uViewMat);
          } catch (err) {}
        } catch (err) {}
      }
      if ((_self$spritz2 = self.spritz) !== null && _self$spritz2 !== void 0 && _self$spritz2.world) self.spritz.world.isPaused = false;
      self._freecamActive = false;
      var _info = document.getElementById('pixos-freecam-info');
      if (_info) document.body.removeChild(_info);
      try {
        document.exitPointerLock = document.exitPointerLock || document.mozExitPointerLock;
        document.exitPointerLock();
      } catch (err) {}
      if (self._freecamHandlers) {
        window.removeEventListener('wheel', self._freecamHandlers.onWheel, {
          capture: true
        });
        document.removeEventListener('pointermove', self._freecamHandlers.onPointerMove, {
          capture: true
        });
        document.removeEventListener('mousemove', self._freecamHandlers.onPointerMove, {
          capture: true
        });
        try {
      ...
      try {
        var _self$_bodyOverflowSa;
        document.body.style.overflow = (_self$_bodyOverflowSa = self._bodyOverflowSaved) !== null && _self$_bodyOverflowSa !== void 0 ? _self$_bodyOverflowSa : '';
      } catch (err) {}
      try {
        var s = document.getElementById('pixos-freecam-status');
