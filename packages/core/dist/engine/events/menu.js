...
var _default = exports["default"] = {
  ...
  tick: function tick(time) {
    ...
    Object.keys(this.menuDict).filter(function (key) {
      ...
      if (section.prompt) {
        if (_this.speechOutput) {
          _this.engine.speechSynthesis(section.prompt);
        }
        _this.textbox = _this.engine.hud.scrollText(section.prompt, _this.scrolling, _this.options);
      }
    });
  ...
  checkInput: function checkInput(time) {
    ...
    if (time > this.lastKey + 200) {
      switch (this.engine.keyboard.lastPressedCode()) {
        ...
        case 'Enter':
          Object.keys(this.menuDict).filter(function (key) {
            return _this3.activeMenus.includes(key);
          }).map(function (id) {
            var section = _this3.menuDict[id];
            if (section.onEnter) section.trigger(_this3);
          });
