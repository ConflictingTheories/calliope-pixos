  ...
  cells: function cells(bounds, zone) {
    ...
    var cells = new Array(height).fill(null).map(function (_, i) {
      return new Array(width).fill(null).map(function (__, j) {
        ...
        if (posKey % Math.abs(3 + (store.pixos && store.pixos[_generator.STORE_NAME] ? store.pixos[_generator.STORE_NAME].selected : 7)) !== 0) {
          return T.FLOOR;
        } // return random tile based on location (x+i, y+j)
        // 66% of tiles are floor


        if (posKey % Math.abs(7 + (store.pixos && store.pixos[_generator.STORE_NAME] ? store.pixos[_generator.STORE_NAME].selected : 7)) !== 0) {
          return T.PILLAR;
  ...
  sprites: function sprites(bounds, zone) {
    ...
    var portals = [{
      ...
      onStep: function onStep() {
        store.pixos[_generator.STORE_NAME].position = _construct(_vector.Vector, [5, 3, 0]);
        store.pixos[_generator.STORE_NAME].selected += 3;
      },
      zones: ["ice"]
    }, {
      id: "door-r",
      ...
      facing: _enums.Direction.Down,
      onStep: function onStep() {
        store.pixos[_generator.STORE_NAME].position = _construct(_vector.Vector, [8, 3, 0]);
        store.pixos[_generator.STORE_NAME].selected += 7;
      },
      zones: ["jungle"]
    }];
    new Array(height).fill(null).map(function (_, i) {
      return new Array(width).fill(null).map(function (__, j) {
        ...
        if (posKey % Math.abs(3 + (store.pixos && store.pixos[_generator.STORE_NAME] ? store.pixos[_generator.STORE_NAME].selected : 7)) !== 0) {
          ...
          if (posKey % Math.abs(5 + (store.pixos && store.pixos[_generator.STORE_NAME] ? store.pixos[_generator.STORE_NAME].selected : 7)) === 0) {
            // add Portals randomly around map on floor tiles
            if (portals.length > 0 && posKey % Math.abs(22) && zone.getHeight(j, i) === 0) {
              var portal = portals.pop();
              portal.pos = _construct(_vector.Vector, [j, i, zone.getHeight(j, i)]);
              posKey % Math.abs(sprites.push(portal));
            } else if (portals.length > 0 && posKey % Math.abs(33) && zone.getHeight(j, i) === 0) {
              var _portal = portals.shift();

              _portal.pos = _construct(_vector.Vector, [j, i, zone.getHeight(j, i)]);
              posKey % Math.abs(sprites.push(_portal));
            }
          }
