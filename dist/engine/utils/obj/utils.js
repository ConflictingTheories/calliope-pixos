  ...
  for (var materialName in mtl.materials) {
    ...
    var _loop_1 = function _loop_1(attr) {
      ...
      textures.push(fetch(url).then(function (response) {
        ...
        mapData.glTexture = texture; // Set the parameters so we can render any size image.
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, mapData.texture); // Upload the image into the texture.
...
function downloadModels(gl, models) {
  ...
  var _loop_2 = function _loop_2(model) {
    ...
    var mtlPromise = void 0; // Download MaterialLibrary file?
  ...
  return Promise.all(finished).then(function (ms) {
    var e_3, _a; // the "finished" promise is a list of name, Mesh instance,
