...
function () {
  ...
  function Mesh(objectData, options) {
    ...
    options.enableWTextureCoord = !!options.enableWTextureCoord; // the list of unique vertex, normal, texture, attributes

    this.vertexNormals = [];
    this.textures = []; // the indicies to draw the faces
    ...
    var materialIndicesByName = {}; // keep track of what material we've seen last

    var currentMaterialIndex = -1;
    var currentObjectByMaterialIndex = 0; // unpacking stuff
    ...
    var FACE_RE = /^f\s/;
    var WHITESPACE_RE = /\s+/;
    var USE_MATERIAL_RE = /^usemtl/; // array of lines separated by the newline
    ...
    try {
      for (var lines_1 = __values(lines), lines_1_1 = lines_1.next(); !lines_1_1.done; lines_1_1 = lines_1.next()) {
        ...
        if (VERTEX_RE.test(line)) {
          ...
          var coords = elements; // by default, the loader will only look at the U and V
          ...
          var materialName = elements[0]; // check to see if we've ever seen it before

          if (!(materialName in materialIndicesByName)) {
            // new material we've never seen
            materialNamesByIndex.push(materialName);
            materialIndicesByName[materialName] = materialNamesByIndex.length - 1; // push new array into indices
          ...
          currentMaterialIndex = materialIndicesByName[materialName]; // update current index array
              ...
              for (var j = 0, eleLen = triangle.length; j < eleLen; j++) {
                ...
                if (hash in unpacked.hashindices) {
                  ...
                  /*
                             ...
                             */
                  var vertex = triangle[j].split("/"); // it's possible for faces to only specify the vertex
                  ...
                  unpacked.norms.push(+vertNormals[(+vertex[normalIndex] - 1) * 3 + 2]); // Vertex material indices

                  unpacked.materialIndices.push(currentMaterialIndex); // add the newly created Vertex to the list of indices

                  unpacked.hashindices[hash] = unpacked.index;
                  unpacked.indices[currentObjectByMaterialIndex].push(unpacked.hashindices[hash]); // increment the counter
  ...
  Mesh.prototype.calculateTangentsAndBitangents = function () {
    ...
    var unpacked = {
      tangents: __spreadArray([], __read(new Array(this.vertices.length)), false).map(function (_) {
        return 0;
      }),
      bitangents: __spreadArray([], __read(new Array(this.vertices.length)), false).map(function (_) {
        return 0;
      })
    ...
    for (var i = 0; i < indices.length; i += 3) {
      ...
      var r = 1.0 / Math.abs(rInv < 0.0001 ? 1.0 : rInv); // Tangent
      ...
      var z_tangent = (z_deltaPos1 * y_uvDeltaPos2 - z_deltaPos2 * y_uvDeltaPos1) * r; // Bitangent
      ...
      var z_bitangent = (z_deltaPos2 * x_uvDeltaPos1 - z_deltaPos1 * x_uvDeltaPos2) * r; // Gram-Schmidt orthogonalize
      ...
      var z_n2 = normals[i2 * 3 + 2]; // Tangent
      ...
      var magTangent2 = Math.sqrt(x_resTangent2 * x_resTangent2 + y_resTangent2 * y_resTangent2 + z_resTangent2 * z_resTangent2); // Bitangent
      ...
      unpacked.bitangents[i2 * 3 + 2] += z_resBitangent2 / magBitangent2; // TODO: check handedness
    }
  ...
  Mesh.prototype.makeIndexBufferDataForMaterials = function () {
    ...
    var indices = (_a = new Array()).concat.apply(_a, __spreadArray([], __read(materialIndices.map(function (mtlIdx) {
      return _this.indicesPerMaterial[mtlIdx];
    })), false));
  ...
  Mesh.prototype.addMaterialLibrary = function (mtl) {
    for (var name_1 in mtl.materials) {
      ...
      var material = mtl.materials[name_1]; // Find the material index for this material

      var materialIndex = this.materialIndices[material.name]; // Put the material into the materialsByIndex object at the right
  ...
  return __generator(this, function (_a) {
    switch (_a.label) {
      ...
      case 3:
        ...
        return [4
        /*yield*/
        , [elements[2], elements[3], elements[0]]];
      ...
      case 6:
        if (!(i < elements.length - 1)) return [3
        /*break*/
        , 9];
        return [4
        /*yield*/
        , [elements[0], elements[i], elements[i + 1]]];
